...
const ProtectedRoute = ({ children, adminOnly = false }: { children: React.ReactNode, adminOnly?: boolean }) => {
  ...
  if (adminOnly && !(profile?.roles?.includes('admin') || profile?.role === 'admin')) {
    return <Navigate to="/" />;
  }
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="z-10 bg-[#111] p-8 rounded-2xl border border-white/10 shadow-2xl shadow-cyan-500/10 max-w-md w-full text-center">
        ...
        <button
          onClick={login}
          className="w-full py-3 px-4 bg-white text-black font-semibold rounded-xl hover:bg-gray-200 transition-colors flex items-center justify-center gap-3"
        >
